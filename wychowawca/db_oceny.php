...
function oceny($id,$id_przed,$id_kl)
{
  ...
  if($result = $mysqli->query("SELECT * FROM oceny_cz_1 WHERE id_user='$id' AND id_przed='$id_przed' AND id_kl='$id_kl' ORDER BY poz"))
  {
	while($row=$result->fetch_object())
	{
...
function opis_oc($id_przed,$id_kl)
{
  ...
  if($result = $mysqli->query("SELECT poz,opis FROM oceny_op WHERE id_przed='$id_przed' AND id_kl='$id_kl' ORDER BY poz"))
  {
	while($row=$result->fetch_object())
	{
...
function prop_sem($id,$id_przed,$id_kl)
{
  ...
  $query = "SELECT prop, data FROM ocen_prop_sem WHERE id_user='$id' AND id_kl='$id_kl' AND id_przed='$id_przed'";
  $tab = zapytanie($query);
  $prop = $tab[0];
  $data = $tab[1];
...
function sem($id,$id_przed,$id_kl)
{
  ...
  $query = "SELECT sem, data FROM ocen_sem WHERE id_user='$id' AND id_kl='$id_kl' AND id_przed='$id_przed'";
  $tab = zapytanie($query);
  $sm = $tab[0];
  $data = $tab[1];
...
{
	 //Miesiące
	 frek_msc($id_ucz,'09',$mysqli);
	 frek_msc($id_ucz,'10',$mysqli);
	 frek_msc($id_ucz,'11',$mysqli);
	 frek_msc($id_ucz,'12',$mysqli);
	 frek_msc($id_ucz,'01',$mysqli);
...
{
	$query = "SELECT zach FROM zach_sem WHERE id_user='$id'";
	$tab = zapytanie($query);
	$zach = $tab[0];
...
<div id="kontener">
  ...
  <li><a href="db_oceny.php?id_ucz=<?php echo $id ;?>" title="Oceny semestr I" class="zaj aktywna">Semestr I</a></li>
  <li><a href="db_oceny_k.php?id_ucz=<?php echo $id ;?>" title="Oceny semestr II" class="zaj">Semestr II</a></li>
  </ul>
  </div>
  <h3 id="nagRamka1">OCENY - SEMESTR I<div id="time"><a href="javascript:drukuj()"><img src="image/printer.png" alt="Drukarka" title="Drukuj"></a></div></h3>
  <?php
  ...
  ?>
  <div id="drukFrek">
  <h3 id="nagRamka6">ZESTAWIENIE FREKWENCJI ZA I SEMESTR</h3>
