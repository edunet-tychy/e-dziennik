  ...
  if(isset($dane))
  {
	foreach($dane as $uczen)
	{
	 ...
	 if(isset($przed))
	 {
	   ...
	   {
		  $query = "SELECT sem FROM ocen_sem WHERE id_user='$uczen[2]' AND id_przed='$prz'";
		  $tab = zapytanie($query);
		  $ocena = $tab[0];
	...
	//Resestowanie sumowania ocen
	  ...
	  $query = "SELECT count(stan) FROM frekwencja WHERE stan='u' AND id_ucz='$uczen[2]' AND (MONTH(data) > 8 OR MONTH(data) < 2)";
	  $tab = zapytanie($query);
	  $usp = $tab[0];
	  
	  echo'<td id="l_str">'.$usp.'</td>';

	  $query = "SELECT count(stan) FROM frekwencja WHERE stan='n' AND id_ucz='$uczen[2]' AND (MONTH(data) > 8 OR MONTH(data) < 2)";
	  $tab = zapytanie($query);
	  $nusp = $tab[0];
	  
	  echo'<td>'.$nusp.'</td>';

	  $query = "SELECT count(stan) FROM frekwencja WHERE stan='s' AND id_ucz='$uczen[2]' AND (MONTH(data) > 8 OR MONTH(data) < 2)";
	  $tab = zapytanie($query);
	  $sp = $tab[0];
...
<div id="kontener">
  ...
  <div id="czescGlowna">
    <div id="lista" class="zawartosc">
      <h3 id="nagRamka">ZESTAWIENIE OCEN ZA I PÓŁROCZE (SEMESTR) - KLASA <?php echo $klasa . ' ' . $sb?></h3>
      <div id="time-2"><a href="javascript:drukuj()"><img src="image/printer.png" alt="Drukarka" title="Drukuj cały formularz"></a></div>
