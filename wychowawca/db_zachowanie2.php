...
for($i=1 ; $i<=$ile ; $i++)
{
	...
	$query = "SELECT id_zach,zach FROM zach_rok WHERE id_user='$id_user' AND id_kl='$id_kl'";
	$tab = zapytanie($query);
	$id_zach = $tab[0];
	$zach_tab = $tab[1];
...
function zach_dod($zach,$id_user,$id_kl)
{
  global $mysqli;
   ...
   {
	  if($stmt = $mysqli->prepare("INSERT zach_rok (zach,id_user,id_kl) VALUES (?,?,?)"))
	  {
...
function zach_pop($zach,$id_user,$id_kl)
{
  global $mysqli;
   ...
   if($zach == '' || $id_user == '' || $id_kl == '')
   {
	  ...
	  if($stmt = $mysqli->prepare("UPDATE zach_rok SET zach = ? WHERE id_user = ? AND id_kl = ?"))
	  {
		 $stmt->bind_param("iii",$zach,$id_user,$id_kl);
		 $stmt->execute();
		 $stmt->close();
...
{
  global $mysqli;
  if($stmt=$mysqli->prepare("DELETE FROM zach_rok WHERE id_zach = ? LIMIT 1"))
  {
