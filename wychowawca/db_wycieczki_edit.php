...
//Dodanie wydarzenia	
	$("#poprawWycieczke").click(function()
	{
...
//Wycieczka
$pyt = "SELECT data, czas, ilu_uczniow, dokad, id_user FROM wycieczki WHERE id_wyk='$id_wyk'";
$tab = zapytanie($pyt);
$data = $tab[0];
$czas = $tab[1];
$ile = $tab[2];
...
$pyt = "SELECT imie, nazwisko FROM users WHERE id='$user'";
$tab = nauczyciel($pyt);
$imie = $tab[0];
$nazwisko = $tab[1];
...
<div id="kontener">
	...
	<div id="czescGlowna">
      ...
      <div id="lista" class="zawartosc">
        <h3 id="nagRamka1">EDYCJA WYCIECZKI - KLASA <?php echo $klasa .' '. $sb; ?></h3>
        <form action="db_wycieczki_upd.php" method="post" name="form" id="form">
        <input type="hidden" name="id_wyk" id="id_wyk" value="<?php echo $id_wyk ?>">
          ...
          <tr><td class="dane-3"><input type="text" id="czas" size="30" class="pole" name="czas" value="<?php echo $czas ?>"></td></tr>
          <tr><td class="dane-3">Liczba uczestników:*</td></tr>
          <tr><td class="dane-3"><input type="text" id="liczba" size="30" class="pole" name="liczba" value="<?php echo $ile ?>"></td></tr>
          <tr><td class="dane-3">Nauczyciel prowadzący wycieczkę:*</td></tr>
          <tr><td class="dane-3">
          <select class="min" name="nauczyciel">
            <?php echo '<option value="' . $user . '">'. $imie . ' ' . $nazwisko . '</option>';?>
        	<option value="0">---</option>
		  	<?php nauczyciele();?>
        </select>
          </td></tr>
          <tr><td class="dane-3">Dokąd i w jakim celu odbyła się wycieczka:*</td></tr>
          <tr><td class="dane-3"><textarea name="cel" rows="2" cols="80" ><?php echo $cel ?></textarea></td></tr>
          <tr>
            <td colspan="2" class="center-3"><input type="button" value="Popraw dane" class="button" id="poprawWycieczke"></td>
          </tr>
          </table>
        </form>
      </div>
