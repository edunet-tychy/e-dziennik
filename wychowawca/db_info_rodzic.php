...
class idUser
{
  public function uczniowie($mysqli,$id_kl)
  {
	...
	$query = "SELECT id_ucz FROM uczen WHERE id_kl = '$id_kl'";
	...
	if($result->num_rows > 0)
	{
	  while($row=$result->fetch_object())
	  {
		 ...
		 $query = "SELECT id_rodz FROM rodzice WHERE id_ucz = '$row->id_ucz'";
		 $baza->pytanie($query);
		 $id_rodz = $baza->tab[0];
		 $query = "SELECT id_user FROM rodzic WHERE id_rodz = '$id_rodz'";
		 $baza->pytanie($query);
		 $tab[]=$baza->tab[0];
	  }		
	}
  ...
  public function tabela($mysqli,$id_kl)
  {
	...
	if(isset($tab))
	{
	  ...
	  foreach($tab as $dane)
	  {
		...
		<td class="loginUser" id="edycja""><a href="db_user_edit.php?id='.$dane[4].'&zw=r"><img src="image/edytuj2.png"></a></td>
		</tr>';
	  }	
	}
  }
