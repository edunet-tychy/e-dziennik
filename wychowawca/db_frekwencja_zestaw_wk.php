...
function frek_msc($id_kl,$m)
{
  ...
  if($result = $mysqli->query("SELECT stan,id_ucz FROM frekwencja WHERE id_kl='$id_kl' AND MONTH(data)='$m'"))
  {
	while($row=$result->fetch_object())
	{
	 ...
	 $result2 = $mysqli->query("SELECT data FROM frekwencja WHERE id_kl='$id_kl' AND (MONTH(data)='$m') GROUP BY data");
...
function frek_sem($id_kl)
{
  ...
  if($result = $mysqli->query("SELECT stan,id_ucz FROM frekwencja WHERE id_kl='$id_kl'"))
  {
	while($row=$result->fetch_object())
	{
		 $stan = $row->stan;
	 ...
	 $result2 = $mysqli->query("SELECT data FROM frekwencja WHERE id_kl='$id_kl' GROUP BY data");
	 while($row2=$result2->fetch_object())
	 {
		$daty[] = $row2->data;
	 }
