...
{
  var url_n = 'db_oceny_opis_k.php?id_zaj=';
...
include_once('../class/oceny_uczniowie_kon.class.php');
include_once('../class/oceny_kon.class.php');
...
function przycisk($id_zaj,$li,$traf,$zaj)
{
  if($li > 1) //Przypadek 1 - nauczyciel prowadzi więcej niż 1 przedmiot
  {
	for($i = 0; $i < $li; $i++)
	{
	  if($zaj[$i] == $id_zaj)
	  {
		echo '<div class="prawy"><div id="zapis"></div><input type="button" value="Zapisz oceny" class="przycisk-oc" id="zapisOcen_k"></div> ';
		//Formularz - id_przed
	...
	if ($traf == 0)
	{
	  echo '<div class="prawy"><input type="button" value="Zapisz oceny" class="przycisk-oc" id="zapisOcen_k" disabled></div> ';
	  $aktywny=0;
	}
...
<div id="kontener">
  ...
  <div id="czescGlowna">
    ...
    <div class="linia-3"></div>
      <form action="db_oceny_k.php" method="post" name="form" id="form">
      <?php
