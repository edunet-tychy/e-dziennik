	...
	if($this->result = $mysqli->query("SELECT id_kp FROM klasy_nauczyciele WHERE id_naucz='$id'"))
	{
	  if($this->result->num_rows > 0)
	  {
		...
		for($i=0; $i < $ile; $i++)
		{
			...
			if($i ==0)
			{
			  echo' <li><a href="#nawigacja_sem" title="klasa" class="zakladki aktywna" id="'.$tab_dz[2].'&id='.$id.'">' .$tab_dz[0]. ' '.$tab_dz[1].'</a></li>';
			} else {
			  echo' <li><a href="#nawigacja_sem" title="klasa" class="zakladki" id="'.$tab_dz[2].'&id='.$id.'">' .$tab_dz[0]. ' '.$tab_dz[1].'</a></li>';
			}
