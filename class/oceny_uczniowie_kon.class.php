		...
		$id_user = $uczen[2];
		  ...
		  if ($nr == 6 || $nr == 11 || $nr == 16 || $nr == 21 || $nr == 26 || $nr == 31)
		  {
			  ...
			  //Pola ocen cząstkowych
				...
				if($this->result3 = $mysqli->query("SELECT prop FROM ocen_prop_rok WHERE id_kl='$id_kl' AND id_user='$id_user' AND poz='$nr' AND id_przed='$id_zaj'"))
				{
				...
				//Oceny semestralne
				if($this->result4 = $mysqli->query("SELECT sem FROM ocen_rok WHERE id_kl='$id_kl' AND id_user='$id_user' AND poz='$nr' AND id_przed='$id_zaj'"))
				{
